...
export const sendNotification = onCall({ region: "asia-southeast1", secrets: [] }, async (request) => {
    ...
    const messagePayload: admin.messaging.Message = {
        ...
        topic: 'all_users' // Placeholder, will be updated
    };
    if (imageUrl) {
        if(messagePayload.notification) messagePayload.notification.imageUrl = imageUrl;
    }
    ...
    if (target === 'all') {
        ...
        sendPromise = messaging.send(messagePayload);
    } else {
        ...
        sendPromise = messaging.sendEachForMulticast({ ...messagePayload, tokens });
    }

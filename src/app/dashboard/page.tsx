...
export default function DashboardPage() {
    ...
    return (
        <div className="flex h-screen w-full items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-[#001F3D] dark:text-foreground font-black text-lg md:text-xl">جاري معالجة البيانات المباشرة...</p>
            </div>
        </div>
    );
  }
  ...
  const renderTransferSummary = (summary: any, successful: any, pending: any) => (
    <CardContent className="space-y-4 md:space-y-6 pt-6 md:pt-10 flex-grow flex flex-col justify-center px-4 md:px-10">
        <div className="text-center p-4 md:p-6 bg-[#E3F2FD] dark:bg-primary/10 rounded-2xl md:rounded-[2rem] border border-[#1B69FF]/5 w-full flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 w-full">
            <div className="p-4 md:p-6 bg-card rounded-2xl md:rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-sm text-green-600">
                    ...
                    {Object.entries(successful).map(([type, s]: [string, any]) => (
                        <div key={type} className="flex justify-between items-center text-xs md:text-sm gap-2">
                            ...
                            <div className="flex items-center gap-2 md:gap-3">
                                <Badge variant="outline" className="rounded-full bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400 border-green-100 dark:border-green-500/20 font-bold text-[9px] md:text-[10px] shrink-0">{s.count} حوالة</Badge>
                                <FormattedAmount amount={s.amount} currency="ج.م" integerClass="font-bold text-xs md:text-sm" currencyClass="text-[9px] md:text-[10px] font-bold" decimals={0} />
                            </div>
                        </div>
                    ))}
                </div>
            </div>
            <div className="p-4 md:p-6 bg-card rounded-2xl md:rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-sm text-yellow-600">
                <h4 className="text-[10px] md:text-xs font-black uppercase text-inherit tracking-widest mb-3 md:mb-4 flex items-center gap-2">
                ...
                <div className="space-y-2 md:space-y-3">
                    {Object.entries(pending).map(([type, s]: [string, any]) => (
                        <div key={type} className="flex justify-between items-center text-xs md:text-sm gap-2">
                            <span className="font-bold text-slate-500 shrink-0">{type}</span>
                            <div className="flex items-center gap-2 md:gap-3">
                                <Badge variant="outline" className="rounded-full bg-yellow-50 dark:bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-100 dark:border-yellow-500/20 font-bold text-[9px] md:text-[10px] shrink-0">{s.count} حوالة</Badge>
                                <FormattedAmount amount={s.amount} currency="ج.م" integerClass="font-bold text-xs md:text-sm" currencyClass="text-[9px] md:text-[10px] font-bold" decimals={0} />
                            </div>
                        </div>
                    ))}
    ...
    <div className="space-y-6 md:space-y-10 pb-10 max-w-full overflow-visible">
        <div className="grid gap-4 md:gap-8 md:grid-cols-2 lg:grid-cols-3">
            <Card className="floating-card flex flex-col p-1 md:p-2 overflow-visible">
                <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                        <div>
                            <CardTitle className="text-[#001F3D] dark:text-foreground font-black text-base md:text-lg text-right">إجمالي الرصيد الليبي</CardTitle>
                        ...
                        <div className="p-3 md:p-4 bg-green-50 dark:bg-green-500/10 rounded-xl md:rounded-[1.5rem] shrink-0 flex items-center justify-center min-w-[50px] md:min-w-[64px]">
            ...
            <Card className="floating-card flex flex-col p-1 md:p-2 overflow-visible">
                <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                        <div><CardTitle className="text-[#001F3D] dark:text-foreground font-black text-base md:text-lg">تداول العملات</CardTitle><CardDescription className="text-[10px] md:text-xs font-bold text-slate-400">العمليات الناجحة (DG)</CardDescription></div>
                        <div className="p-3 md:p-4 bg-blue-50 dark:bg-primary/10 rounded-xl md:rounded-[1.5rem] shrink-0"><ArrowRightLeft className="h-5 w-5 md:h-6 md:w-6 text-[#1B69FF]" /></div>
                    </div>
                </CardHeader>
                <CardContent className="space-y-4 md:space-y-6 flex-grow flex flex-col justify-center py-4 md:py-6 px-4 md:px-6 overflow-visible">
                    <div className="space-y-2 md:space-y-3 overflow-visible">
                        ...
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">العمليات</span><span className="font-bold text-[#001F3D] dark:text-foreground text-sm md:text-base text-left flex-1" dir="ltr">{stats.dailyTradeStats.count}</span></div>
                        ...
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">المبلغ المصري</span><FormattedAmount amount={stats.dailyTradeStats.egpAmount} currency="ج.م" integerClass="font-bold text-xs md:text-sm" currencyClass="text-[9px] md:text-[10px] font-bold" className="justify-start w-full" decimals={0} /></div>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">حصالة الفكة</span><FormattedAmount amount={stats.dailyTradeStats.fakkaAmount} currency="ج.م" integerClass="font-bold text-xs md:text-sm text-orange-600" currencyClass="text-[9px] md:text-[10px] font-bold" className="justify-start w-full" decimals={2} /></div>
                    </div>
                    ...
                    <div className="space-y-2 md:space-y-3 overflow-visible">
                        ...
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">العمليات</span><span className="font-bold text-[#001F3D] dark:text-foreground text-sm md:text-base text-left flex-1" dir="ltr">{stats.monthlyTradeStats.count}</span></div>
                        ...
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">المبلغ المصري</span><FormattedAmount amount={stats.monthlyTradeStats.egpAmount} currency="ج.م" integerClass="font-bold text-xs md:text-sm" currencyClass="text-[9px] md:text-[10px] font-bold" className="justify-start w-full" decimals={0} /></div>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">حصالة الفكة</span><FormattedAmount amount={stats.monthlyTradeStats.fakkaAmount} currency="ج.م" integerClass="font-bold text-xs md:text-sm text-orange-600" currencyClass="text-[9px] md:text-[10px] font-bold" className="justify-start w-full" decimals={2} /></div>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card className="floating-card flex flex-col p-1 md:p-2 overflow-visible">
                <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                        <div>
                            <CardTitle className="text-[#001F3D] dark:text-foreground font-black text-base md:text-lg text-right">إجمالي الرصيد المصري</CardTitle>
                        ...
                        <div className="p-3 md:p-4 bg-purple-50 dark:bg-purple-500/10 rounded-xl md:rounded-[1.5rem] shrink-0 flex items-center justify-center min-w-[50px] md:min-w-[64px]">
        ...
        <div className="grid gap-4 md:gap-8 md:grid-cols-2 lg:grid-cols-3">
            <Card className="floating-card flex flex-col p-1 md:p-2 overflow-visible">
                <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                        <div><CardTitle className="text-[#001F3D] dark:text-foreground font-black text-base md:text-lg">إدارة المستخدمين</CardTitle><CardDescription className="text-[9px] md:text-[10px] text-yellow-500 font-black uppercase tracking-widest">{stats.userCounts.pendingDoc} طلب توثيق</CardDescription></div>
                        <div className="p-3 md:p-4 bg-orange-50 dark:bg-orange-500/10 rounded-xl md:rounded-[1.5rem] shrink-0"><Users2 className="h-5 w-5 md:h-6 md:w-6 text-orange-600" /></div>
                    </div>
                </CardHeader>
                <CardContent className="flex-grow flex flex-col justify-center py-4 md:py-6 px-4 md:px-6 overflow-visible">
                    <div className="text-center text-4xl md:text-6xl font-black text-[#001F3D] dark:text-foreground mb-6 md:mb-8 tabular-nums">{stats.userCounts.total}</div>
                    <div className="grid grid-cols-2 gap-2 md:gap-3">
                        <div className="flex justify-between items-center bg-green-50 dark:bg-green-500/10 p-2 md:p-3 rounded-xl md:rounded-2xl border border-green-100 dark:border-green-500/20"><span className="text-[8px] md:text-[10px] font-black text-green-700 dark:text-green-400 uppercase">تاجر</span><span className="font-bold text-green-700 dark:text-green-400 text-xs md:text-base">{stats.userCounts.merchants}</span></div>
                        <div className="flex justify-between items-center bg-blue-50 dark:bg-primary/10 p-2 md:p-3 rounded-xl md:rounded-2xl border border-blue-100 dark:border-primary/20"><span className="text-[8px] md:text-[10px] font-black text-blue-700 dark:text-blue-400 uppercase">موثق</span><span className="font-bold text-blue-700 dark:text-blue-400 text-xs md:text-base">{stats.userCounts.verified}</span></div>
                        <div className="flex justify-between items-center bg-yellow-50 dark:bg-yellow-500/10 p-2 md:p-3 rounded-xl md:rounded-2xl border border-yellow-100 dark:border-yellow-500/20"><span className="text-[8px] md:text-[10px] font-black text-yellow-700 dark:text-yellow-400 uppercase">غير موثق</span><span className="font-bold text-yellow-700 dark:text-yellow-400 text-xs md:text-base">{stats.userCounts.unverified}</span></div>
                        <div className="flex justify-between items-center bg-red-50 dark:bg-red-500/10 p-2 md:p-3 rounded-xl md:rounded-2xl border border-red-100 dark:border-red-500/20"><span className="text-[8px] md:text-[10px] font-black text-red-700 dark:text-red-400 uppercase">مجمد</span><span className="font-bold text-red-700 dark:text-red-400 text-xs md:text-base">{stats.userCounts.banned}</span></div>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card className="floating-card flex flex-col p-1 md:p-2 overflow-visible">
                ...
                <CardContent className="space-y-4 md:space-y-6 flex-grow flex flex-col justify-center py-4 md:py-6 px-4 md:px-6 overflow-visible">
                    <div className="space-y-2 md:space-y-3 overflow-visible">
                        <h4 className="mb-2"><InlineDaySelector /></h4>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">العمليات</span><span className="font-bold text-[#001F3D] dark:text-foreground text-sm md:text-base text-left flex-1" dir="ltr">{stats.dailyInternalStats.count}</span></div>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">إجمالي الرسوم</span><FormattedAmount amount={stats.dailyInternalStats.revenue} currency="د.ل" integerClass="font-bold text-xs md:text-sm text-green-600" className="justify-start w-full" decimals={0} /></div>
                    </div>
                    <Separator className="bg-slate-50 dark:bg-white/5" />
                    <div className="space-y-2 md:space-y-3 overflow-visible">
                        <h4 className="mb-2"><InlineMonthSelector /></h4>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">العمليات</span><span className="font-bold text-[#001F3D] dark:text-foreground text-sm md:text-base text-left flex-1" dir="ltr">{stats.monthlyInternalStats.count}</span></div>
            ...
            <Card className="floating-card flex flex-col p-1 md:p-2 overflow-visible">
                ...
                <CardContent className="space-y-4 md:space-y-6 flex-grow flex flex-col justify-center py-4 md:py-6 px-4 md:px-6 overflow-visible">
                    <div className="space-y-2 md:space-y-3 overflow-visible">
                        <h4 className="mb-2"><InlineDaySelector /></h4>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">عدد الكروت</span><span className="font-bold text-[#001F3D] dark:text-foreground text-sm md:text-base text-left flex-1" dir="ltr">{stats.dailyCardStats.count}</span></div>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">قيمة المبيعات</span><FormattedAmount amount={stats.dailyCardStats.value} currency="د.ل" integerClass="font-bold text-xs md:text-sm text-sky-600" className="justify-start w-full" decimals={0} /></div>
                    </div>
                    <Separator className="bg-slate-50 dark:bg-white/5" />
                    <div className="space-y-2 md:space-y-3 overflow-visible">
                        <h4 className="mb-2"><InlineMonthSelector /></h4>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">عدد الكروت</span><span className="font-bold text-[#001F3D] dark:text-foreground text-sm md:text-base text-left flex-1" dir="ltr">{stats.monthlyCardStats.count}</span></div>
                        <div className="flex justify-between items-center gap-2"><span className="text-slate-500 font-bold text-xs md:text-sm shrink-0">قيمة المبيعات</span><FormattedAmount amount={stats.monthlyCardStats.value} currency="د.ل" integerClass="font-bold text-xs md:text-sm text-sky-600" className="justify-start w-full" decimals={0} /></div>
                    </div>
                </CardContent>
            </Card>
        </div>
                ...
                <CardHeader className="pb-2 md:pb-4">
                    <div className="flex items-start justify-between">
                        <div className="flex flex-col space-y-1">
                            <CardTitle className="text-[#001F3D] dark:text-foreground font-black text-base md:text-lg text-right">رسوم التحويلات المصرية</CardTitle>
                        ...
                        <div className="p-3 md:p-4 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl md:rounded-[1.5rem] shadow-inner shadow-indigo-600/5 shrink-0">
                            <Banknote className="h-5 w-5 md:h-6 md:w-6 text-indigo-600" />
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="space-y-4 md:space-y-6 flex-grow flex flex-col justify-center py-4 md:py-6 px-4 md:px-6 overflow-visible">
                    <div className="text-center py-3 md:py-4 bg-indigo-50/50 dark:bg-indigo-500/10 rounded-xl md:rounded-[1.5rem] border border-indigo-100/50 dark:border-white/5 w-full overflow-visible px-6 flex items-center justify-center">
                    ...
                    <div className="space-y-3 md:space-y-4 overflow-visible">
                        {Object.entries(stats.monthlyRevenueByType).map(([type, s]: [string, any]) => (
                            <div key={type} className="flex justify-between items-center text-xs md:text-sm gap-2">
                                <span className="font-bold text-slate-500 shrink-0">{type}</span>
                                <div className="flex items-center gap-2 md:gap-3">
                                    <Badge variant="outline" className="rounded-full bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-white/5 font-bold text-[9px] md:text-[10px] px-1.5 md:px-2 shrink-0">{s.count}</Badge>
                                    <FormattedAmount amount={s.revenue} currency="ج.م" integerClass="font-bold text-[#001F3D] dark:text-foreground text-xs md:text-sm" className="justify-start" decimals={0} />
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <Tabs defaultValue="today" dir="rtl" className="flex flex-col h-full">
                    <CardHeader className="pb-2">
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <CardTitle className="flex items-center gap-2 md:gap-3 text-[#001F3D] dark:text-foreground font-black text-lg md:text-xl">
                                <TrendingUp className="h-5 w-5 md:h-6 md:w-6" /> 
                                ملخص الحوالات المصرية
                            </CardTitle>
                            <TabsList className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl md:rounded-2xl h-10 md:h-12 w-full sm:w-auto">
                                <TabsTrigger value="today" className="flex-1 sm:flex-none rounded-lg md:rounded-xl px-4 md:px-6 font-black text-[10px] md:text-xs uppercase tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-slate-900 data-[state=active]:shadow-sm">تقرير اليوم</TabsTrigger>
                                <TabsTrigger value="month" className="flex-1 sm:flex-none rounded-lg md:rounded-xl px-4 md:px-6 font-black text-[10px] md:text-xs uppercase tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-slate-900 data-[state=active]:shadow-sm">تقرير الشهر</TabsTrigger>
                            </TabsList>
                        </div>
                    </CardHeader>
        ...
        <Card className="floating-card p-2 md:p-4 overflow-hidden">
            <CardHeader className="pb-4 md:pb-8">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3 md:gap-4">
                        <div className="p-3 md:p-4 bg-[#E3F2FD] dark:bg-primary/10 rounded-xl md:rounded-[1.5rem] shrink-0"><BarChart3 className="h-5 w-5 md:h-6 md:w-6 text-[#1B69FF]" /></div>
                        <div>
                            <CardTitle className="text-[#001F3D] dark:text-foreground font-black text-lg md:text-2xl">أداء المندوبين</CardTitle>
            ...
            <CardContent className="px-1 md:px-6 overflow-hidden">
                <div className="rounded-2xl md:rounded-[2rem] border border-slate-100 dark:border-white/5 overflow-x-auto shadow-inner bg-slate-50/30 dark:bg-slate-900/20 w-full">
                    <Table className="min-w-[600px] md:min-w-full">
                        <TableHeader className="bg-card dark:bg-slate-900">
                            <TableRow className="border-b border-slate-100 dark:border-white/5 hover:bg-transparent">
                                <TableHead className="font-black text-[#001F3D] dark:text-slate-300 text-center uppercase tracking-widest text-[9px] md:text-[10px]">المندوب</TableHead>
                                <TableHead className="text-center font-black text-[#001F3D] dark:text-slate-300 uppercase tracking-widest text-[9px] md:text-[10px]">تداول اليوم</TableHead>
                                <TableHead className="text-center font-black text-[#001F3D] dark:text-slate-300 uppercase tracking-widest text-[9px] md:text-[10px]">إجمالي الشهر</TableHead>
                                <TableHead className="text-center font-black text-[#001F3D] dark:text-slate-300 uppercase tracking-widest text-[9px] md:text-[10px]">إجمالي الرسوم</TableHead>
                                <TableHead className="text-center font-black text-[#001F3D] dark:text-slate-300 uppercase tracking-widest text-[9px] md:text-[10px]">العمليات</TableHead>
                                {E_TYPES.map(t => <TableHead key={t} className="text-center font-black text-[#001F3D] dark:text-slate-300 uppercase tracking-widest text-[9px] md:text-[10px]">{t}</TableHead>)}
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {stats.supervisorSummary.map(s => (
                                <TableRow key={s.id} className="hover:bg-[#E3F2FD]/30 dark:hover:bg-slate-800/50 transition-colors border-b border-slate-50/50 dark:border-white/5 last:border-0">
                                    <TableCell className="font-black text-[#001F3D] dark:text-foreground text-center text-xs md:sm">{s.name}</TableCell>
                                    <TableCell className="text-center"><FormattedAmount amount={s.dailyTotal} currency="ج.م" integerClass="font-bold text-slate-600 dark:text-slate-400 text-xs md:text-sm" currencyClass="text-[8px] md:text-[10px] font-bold" className="justify-start" decimals={0} /></TableCell>
                                    <TableCell className="text-center"><FormattedAmount amount={s.monthlyTotal} currency="ج.م" integerClass="font-bold text-[#001F3D] dark:text-foreground text-xs md:text-sm" currencyClass="text-[8px] md:text-[10px] font-bold" className="justify-start" decimals={0} /></TableCell>
                                    <TableCell className="text-center"><FormattedAmount amount={s.monthlyFees} currency="ج.م" integerClass="font-bold text-indigo-600 dark:text-indigo-400 text-xs md:text-sm" currencyClass="text-[8px] md:text-[10px] font-bold" className="justify-start" decimals={0} /></TableCell>

  ...
  return (
    <div className="space-y-6" dir="rtl">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto flex-1">
            <div className="relative flex-1 min-w-[200px] max-w-md">
                ...
                <Input 
                    ...
                    className="pr-10 h-11 rounded-xl bg-card border-slate-200 dark:border-white/10" 
                />
            </div>
            <div className="flex items-center gap-2">
                <Select value={typeFilter} onValueChange={setTypeFilter}>
                    <SelectTrigger className="w-[160px] h-11 rounded-xl bg-card border-slate-200 dark:border-white/10">
          ...
          <Table key={tableKey} ref={tableRef}>
            <TableHeader className="sticky top-0 z-20 bg-slate-50 dark:bg-slate-900 border-b dark:border-white/5 shadow-sm">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="font-black text-[#1B69FF] dark:text-slate-300 text-[10px] uppercase tracking-widest text-center h-12">النوع</TableHead>
                <TableHead className="font-black text-[#1B69FF] dark:text-slate-300 text-[10px] uppercase tracking-widest text-right h-12">التوقيت</TableHead>
                <TableHead className="font-black text-[#1B69FF] dark:text-slate-300 text-[10px] uppercase tracking-widest text-right h-12">صورة</TableHead>
                <TableHead className="text-left font-black text-[#1B69FF] dark:text-slate-300 text-[10px] uppercase tracking-widest h-12">إجراءات</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredData.length === 0 ? (
                ...
                <TableRow key={notification.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors border-b dark:border-white/5 last:border-0">
                  <TableCell className="py-4">
                      <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col text-right">
                              <span className="font-black text-sm text-[#001F3D] dark:text-foreground">{notification.title}</span>
                              <span className="text-[11px] text-slate-400 font-bold line-clamp-1 max-w-[250px]">{notification.body}</span>
                          </div>
                      </div>
                  </TableCell>
                  <TableCell>
                      <Badge variant="outline" className="rounded-full bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-white/5 font-bold text-[10px] px-3">
                  ...
                  <TableCell>
                      {notification.imageUrl ? (
                          <a href={notification.imageUrl} target="_blank" rel="noopener noreferrer" className="relative block h-10 w-10 overflow-hidden rounded-lg border dark:border-white/10 bg-slate-50 dark:bg-slate-900 hover:scale-110 transition-transform">
                  ...
                  <TableCell className="text-left">
                       <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="rounded-2xl border-none shadow-2xl p-2 min-w-[140px] bg-card">
      ...
      <AlertDialog open={!!notificationToDelete} onOpenChange={(open) => !open && setNotificationToDelete(null)}>
          <AlertDialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8 bg-card" dir="rtl">
              <AlertDialogHeader>
              ...
              <AlertDialogFooter className="gap-3 pt-6 flex flex-col sm:flex-row">
                  <AlertDialogCancel onClick={() => setNotificationToDelete(null)} className="rounded-xl font-bold border-slate-200 dark:border-white/10 flex-1 h-12">إلغاء</AlertDialogCancel>
